...
public class LinkedListTabulatedFunctionTest {
    ...
    @Test
    public void testAll() {
        TabulatedFunction first = new LinkedListTabulatedFunction(new double[]{1, 3, 100}, new double[]{2, 6, 200});
        int actual = ((LinkedListTabulatedFunction) first).floorIndexOfX(50);
        int expected = 1;
        assertEquals(actual, expected);
        System.out.println(first.getY(2));

        double actualA = ((LinkedListTabulatedFunction) first).extrapolateLeft(40);
        double expectedA = 80;
        assertEquals(actualA, expectedA);

        double actualB = ((LinkedListTabulatedFunction) first).extrapolateRight(40);
        double expectedB = 80;
        assertEquals(actualB, expectedB);

        double actualC = ((LinkedListTabulatedFunction) first).interpolate(40, 1);
        double expectedC = 80;
        ...
        TabulatedFunction second = new LinkedListTabulatedFunction(new SqrFunction(), 14, 14, 1);
        assertEquals(second.apply(14), 196);
        assertEquals(((LinkedListTabulatedFunction) second).extrapolateLeft(13), 196);
